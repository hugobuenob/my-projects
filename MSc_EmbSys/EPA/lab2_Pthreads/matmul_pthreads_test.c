...
int main() {
	...
	FILE *fp; // Pointer to a file to save data
	fp = fopen ("test_results_pthreads.csv","w+");
	fprintf(fp, "size,threads_number,execution_time\n");
	
	// Thread test ------------------------------------
	for (size = 64; size < 1025; size *= 2){ //Matrix_size_loop from 64 to 1024
		
		printf("-----------------Matrix multiplication (%zux%zu) test-----------------\n", size, size);
		...
		for(threads_number = 1;threads_number < 17; threads_number *= 2){ //Threads_loop --> from 1 to 16
			//Thread implementation
			args = malloc(threads_number * sizeof *args); //Struct with the arguments for pthreads function
			//System operation 				
			...
			for(i = 0; i < threads_number; i ++){
				...
				args[i].id = i;
				args[i].size = size;
				args[i].threads_number = threads_number;
				args[i].a = a;
				args[i].b = b;
				args[i].c = caux;
			...
			t = ((tf.tv_sec - t0.tv_sec) * 1000.0) + ((tf.tv_usec - t0.tv_usec) / 1000.0);
			printf("Execution time for %d threads =: %.3f ms\n", threads_number,t);
			
			fprintf(fp, "%zu,%d,%.3f\n",size,threads_number,t); //Save data 
			free(args);	
