...
PROCESS(sensor_publish_process, "Sensor publish process");
AUTOSTART_PROCESSES(&mqtt_client_process, &sensor_publish_process);
/*---------------------------------------------------------------------------*/
/**
...
//                     conf.event_type_id);
  int len = snprintf(pub_topic, BUFFER_SIZE, "cookie/humtemp");
...
//                     conf.cmd_type);
  int len = snprintf(sub_topic, BUFFER_SIZE, "cookie/config");
...
publish(void)
{
  ...
  len = snprintf(buf_ptr, APP_BUFFER_SIZE, "Test MQTT");
...
PROCESS_THREAD(sensor_publish_process, ev, data)
{
  ...
  PROCESS_BEGIN();
  	...
  	// T/H, set as out pin (PushPull) and enable
  	...
  	while(1){
		...
		printf("Humidity: %lu.%lu %%", hume/1000, hume%1000);
		snprintf(pub_topic, BUFFER_SIZE, "Humidity: %lu.%lu %%", hume/1000, hume%1000);
		publish();
		etimer_reset(&timer);
		PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&timer));
		SI7021_measure(&hume, &temper);
		printf("Temperature: %lu.%lu C", temper/1000, temper%1000);
		snprintf(pub_topic, BUFFER_SIZE, "Temperature: %lu.%lu C", temper/1000, temper%1000);
		publish();
		etimer_reset(&timer);
  	}
